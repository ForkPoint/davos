  ...
  class BMTools {
    constructor (config, ConfigManagerInstance) {
      ...
      this.baseOptions = {
        ...
        form: {
            LoginForm_Login: this.config.username,
            LoginForm_Password: this.config.password
        },
        jar: true,
        ignoreErrors: true,
        followRedirect: true
     ...
     */
    bmRequest (options, path, attemptsLeft, retryDelay, reject, resolve) {
      const self = this;
      ...
      option.url = self.appendCSRF(option.url);
      ...
      req = request(options, function (error, response, body) {
        ...
        if (_.contains(ERR_CODES, error.code)) {
          ...
          // schedule a retry
          setTimeout((function () {
            self.bmRequest(options, path, --attemptsLeft, retryDelay, reject, resolve);
          }), retryDelay);
    ...
    login () {
      ...
      return new Promise(function (resolve, reject) {
        ...
        self.bmRequest(options, null, MAX_ATTEMPTS, RETRY_DELAY, reject, resolve);
      });
    }
